...
class StudentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data       =   Student::join('users', 'users.id', '=', 'students.user_id')->select(
                ...
                'students.is_completed as is_completed',
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['added_by', 'certification_completed', 'action'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        $studentApplication->course_fixed_price =   $request->fix_price;
        $studentApplication->course_duration    =   $request->duration;
    ...
    public function updateStudentCertificateStatus(Request $request)
    {
        $request->validate([
        ...
        if (!$studentCourse) {
            return redirect()->back()->with('error', 'No course found for this student.');
        }

        $totalPaid = StudentPayment::where('student_course_id', $studentCourse->id)->sum('pay');

        if ($request->has('is_completed')) {

            if ($totalPaid >= $studentCourse->course_fixed_price) {

                Student::where('id', $studentId)->update(['is_completed' => 1]);

                return redirect()->back()->with('success', 'Student marked as course completed.');
            } else {
                return redirect()->back()->with('error', 'Cannot mark as completed: course fee is still pending. Paid: ' . $totalPaid . ', Required: ' . $studentCourse->course_fixed_price);
            }
        } else {
            Student::where('id', $studentId)->update(['is_completed' => 0]);

            return redirect()->back()->with('success', 'Course completion status updated.');
        }
    }
